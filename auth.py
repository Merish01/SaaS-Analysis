...
def _fetch_user(access_token: str) -> dict:
    ...
    if resp.status_code != 200:
        raise Exception(f"User fetch failed: {resp.status_code} - {resp.text}")
    return resp.json()
...
def login_page():
    ...
    if code:
        # Skip state check on Streamlit Cloud (session state can reset between redirects)
        ...
        <a href="{auth_url}" class="google-btn" target="_self">
            <svg width="20" height="20" viewBox="0 0 48 48">
              <path fill="#EA4335" d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38
                30.47 0 24 0 11.27 0 .88 8.88.05 20.53L8.73 27c2.09-6.25 7.93-10.8
                14.76-11.05H24z"/>
              ...
              <path fill="#FBBC05" d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14
                .76-4.59l-8.68-6.47C.49 16.35 0 20.1 0 24c0 3.9.49 7.65 1.85
                11.06l8.68-6.47z"/>
              <path fill="#34A853" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6
                c-2.15 1.45-4.92 2.3-8.16 2.3-6.83 0-12.67-4.55-14.76-10.8l-8.68
                6.47C5.99 44.08 14.47 48 24 48z"/>
            </svg>
            &nbsp; Sign in with Google
        </a>
        """, unsafe_allow_html=True)
